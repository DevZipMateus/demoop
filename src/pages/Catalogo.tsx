      ...
      <main className="flex-1 pt-28 sm:pt-32 lg:pt-36">
        <div className="container mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 py-8 sm:py-12 lg:py-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4 lg:gap-6">
            {products.map(product => <div key={product.id} className="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 overflow-hidden border border-demoop-green/10">
                <div className="aspect-square relative overflow-hidden">
                  <img src={product.image} alt={`Produto R$ ${cleanPrice(product.price)}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" onError={e => {
                ...
                <div className="p-3 sm:p-4 text-center">
                  ...
                  <button onClick={() => handleWhatsAppClick(product.price)} className="w-full bg-demoop-primary hover:bg-demoop-darkgreen text-white py-2 px-3 rounded-lg text-sm font-medium transition-all duration-300 hover:shadow-md">
                    Consultar
                  </button>
                </div>
              </div>)}
