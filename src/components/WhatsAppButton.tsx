  ...
  return (
    <button
      ...
      className={`fixed bottom-6 right-6 z-50 flex items-center justify-center w-16 h-16 rounded-full shadow-xl transition-all duration-500 
        ...
        ${isPulsing ? 'animate-bounce shadow-demoop-green/50 shadow-2xl scale-110' : ''}
        hover:scale-110 hover:shadow-2xl bg-demoop-green hover:bg-demoop-darkgreen`}
      style={{
        boxShadow: isPulsing ? '0 0 30px rgba(78, 185, 147, 0.7)' : '0 10px 25px rgba(0,0,0,0.2)',
      }}
      aria-label="Contate-nos pelo WhatsApp"
    >
      <div className={`absolute -inset-3 bg-demoop-green/30 rounded-full ${isPulsing ? 'animate-ping' : 'opacity-0'}`}></div>
      <img 
