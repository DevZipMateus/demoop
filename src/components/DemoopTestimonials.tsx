...
const DemoopTestimonials = () => {
  ...
  const renderStars = (rating: number) => {
    return Array.from({
      length: 5
    }, (_, index) => <span key={index} className={`text-2xl ${index < rating ? 'text-yellow-400' : 'text-gray-300'}`}>
        ★
      </span>);
  };
  return <section id="testimonials" className="py-12 sm:py-16 lg:py-20 xl:py-24 px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 max-w-7xl mx-auto bg-gradient-to-br from-demoop-lightblue/20 to-demoop-lightgreen/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        {testimonials.map((testimonial, index) => <Card key={index} className="bg-white/90 backdrop-blur-sm border-demoop-green/20 shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-105 group">
